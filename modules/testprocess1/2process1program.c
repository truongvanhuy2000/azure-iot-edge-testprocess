...
static int method_callback(const char* method_name, const unsigned char* payload, size_t size, unsigned char** response, size_t* response_size, void* userContextCallback){
    ...
    sprintf(tempMessage, "%d : %s called with %s \n", pid, method_name, payload);
...
void iotHubProccess2(){
    fprintf(stderr, "Start proccess 2\n");
    IOTHUB_CLIENT_TRANSPORT_PROVIDER protocol = MQTT_Protocol;
    IoTHub_Init();
    IOTHUB_MODULE_CLIENT_HANDLE moduleHandler = IoTHubModuleClient_CreateFromEnvironment(protocol);
    if (moduleHandler == NULL){
...
void forkChild(){
    ...
    fprintf(stderr, "Hey, im'starting here");
    ...
    if (pid1 < 0) {
        ...
        iotHubProccess1();
        fprintf(stderr, "First child process: PID = %d\n", getpid());
    } else {
        // This code is executed by the parent process
        fprintf(stderr, "Parent process: PID = %d, First Child PID = %d\n", getpid(), pid1);
        ...
        if (pid2 < 0) {
            ...
            fprintf(stderr, "Second child process: PID = %d\n", getpid());
            iotHubProccess2();
        } else {
            // This code is executed by the parent process
            fprintf(stderr, "Parent process: PID = %d, Second Child PID = %d\n", getpid(), pid2);
            waitpid(pid1, NULL, 0);
            waitpid(pid2, NULL, 0);

            fprintf(stderr, "Parent process: Child processes have finished.\n");
        }
    }
}

int main() {
    (void)fprintf(stderr, "Start 2 girl 1 cup\n");
    forkChild();
    return 0;
}
